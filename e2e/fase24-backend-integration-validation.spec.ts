...
test.describe('🚀 FASE 2.4: Validación de Integración Backend', () => {
  ...
  test('Verificar conectividad completa del backend NestJS', async ({ page }) => {
    ...
    // Navegar a la aplicación (ajustar puerto según donde esté corriendo)
    ...
    await expect(page.locator('[data-testid="main-navigation"], .MuiAppBar-root, nav').first()).toBeVisible();
  ...
  test('Validar endpoints públicos del backend', async ({ page }) => {
    ...
    // Debe haber al menos un request al backend (puede ser cero si usa fallbacks)
    console.log(`ℹ️ Requests detectados: ${backendRequests.length} (fallbacks activos si es 0)`);
