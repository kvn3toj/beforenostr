...
export class MonitoringService {
  ...
  constructor(
    ...
    this.loggerService.log('MonitoringService initialized', 'MonitoringService', {
      videoItemsServiceConnected: !!this.videoItemsService,
      cacheServiceConnected: !!this.cacheService,
      notificationServiceConnected: !!this.notificationService,
      questionValidationServiceConnected: !!this.questionValidationService
    });
  }
  ...
  async handleWeeklyHealthReport() {
    this.loggerService.log('📊 Generating weekly health report...', 'MonitoringService');
    ...
    try {
      ...
      if (reportSent) {
        ...
        this.loggerService.log('Weekly health report sent successfully', 'MonitoringService');
      } else {
        this.addRecentAlert('email', 'Failed to send weekly health report', 'failed');
        this.loggerService.error('Failed to send weekly health report', '', 'MonitoringService');
      }
      
    } catch (error) {
      this.loggerService.logErrorWithContext(error, 'MonitoringService', { operation: 'weekly_health_report' });
    }
  }
  ...
  async handleDailyQuestionValidation() {
    this.loggerService.log('🔍 Starting scheduled daily question validation...', 'MonitoringService');
    ...
    try {
      ...
      this.loggerService.logErrorWithContext(error, 'MonitoringService', { operation: 'daily_question_validation' });
      
      // Enviar alerta de error crítico
      try {
        this.addRecentAlert('email', 'Critical error during question validation', 'failed');
      } catch (alertError) {
        this.loggerService.error('Failed to log question validation error alert', alertError.message, 'MonitoringService');
      }
    }
  }
  ...
  async runConsistencyCheck(): Promise<ConsistencyCheckResultDto> {
    ...
    try {
      ...
      const verificationResult = await this.videoItemsService.verifyAllDurations();
  ...
  private async checkYouTubeApiHealth() {
    ...
    if (configured) {
      try {
        ...
        const testResult = await this.videoItemsService.testFullDurationFlow('dQw4w9WgXcQ');
        accessible = !testResult.error;
  ...
  recordPerformanceMetric(
    ...
    this.loggerService.logPerformance(operation, duration, 'MonitoringService', {
      method,
      success,
      videoId
    });
  ...
  async generateSystemHealthReport(period: 'daily' | 'weekly' | 'monthly'): Promise<SystemHealthReport> {
    const startTime = Date.now();
    this.loggerService.log(`Generating ${period} system health report...`, 'MonitoringService');
    ...
    try {
      ...
      this.loggerService.logPerformance('generateSystemHealthReport', executionTime, 'MonitoringService', {
        period,
      ...
      this.loggerService.logErrorWithContext(error, 'MonitoringService', { 
