...
export const UPlayGamifiedDashboard: React.FC = () => {
  ...
  // NUEVO: Preparar datos para DynamicMetricsDashboard
  const dynamicMetricsData = React.useMemo(() => {
    // Datos de métricas actuales
    const metricsData = {
      meritos: playerMetrics.meritos || 340, // fallback para demo
      ondas: playerMetrics.ondas || 125, // fallback para demo  
      nivel: playerMetrics.level || 1,
      precision: playerMetrics.accuracy || 87, // porcentaje de precisión
      racha: playerMetrics.streak || 5, // días consecutivos
      videosCompletados: playerMetrics.completedVideos || 2,
      tiempoTotal: playerMetrics.totalWatchTime || 95, // minutos
      preguntasRespondidas: playerMetrics.questionsAnswered || 18,
      logrosDesbloqueados: unlockedAchievements.length || 4,
      rankingComunidad: playerMetrics.communityRank || 42, // posición en la comunidad
    };
            ...
            <Grid container spacing={3} maxWidth="800px" mx="auto">
              <Grid size={{ xs: 12, md: 4 }}>
                <Card sx={{ textAlign: 'center', p: 3 }}>
                  <Party sx={{ fontSize: 40, color: 'primary.main', mb: 2 }} />
