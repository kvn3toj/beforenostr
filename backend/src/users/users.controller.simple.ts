...
export class SimpleUsersController {
  constructor(private readonly usersService: SimpleUsersService) {
  ...
  @Get('test')
  @ApiOperation({
    summary: 'Test endpoint para verificar que el controlador simple funciona',
  })
  async test() {
  ...
  @Get('db-test')
  @ApiOperation({
    summary: 'Test directo de la base de datos con controlador simple',
  })
  async dbTest() {
    try {
