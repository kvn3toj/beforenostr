...
//     console.log('>>> LetsService.getTrustRatings: Getting trust ratings for user', userId);
    ...
    try {
      ...
      return {
        ...
        ratings: mockRatings,
        breakdown: {
          communication: mockRatings.reduce((sum, r) => sum + r.communicationRating, 0) / mockRatings.length,
          delivery: mockRatings.reduce((sum, r) => sum + r.deliveryRating, 0) / mockRatings.length,
          quality: mockRatings.reduce((sum, r) => sum + r.qualityRating, 0) / mockRatings.length
        }
      };
    } catch (error) {
...
//     console.log('>>> LetsService.createTrustRating: Creating trust rating', ratingData);
    ...
    try {
      ...
      const mockRating = {
        ...
        communicationRating: ratingData.communicationRating || ratingData.rating,
      ...
      const mockExchanges = [
        {
          ...
          description: 'Sesión práctica sobre cómo aplicar los principios de reciprocidad en intercambios comerciales',
          teacherId: 'teacher1',
          ...
          scheduledAt: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString(), // En 7 días
          status: 'scheduled'
        },
        {
          id: 'ke2',
          title: 'Gestión de Mëritos y Reconocimientos',
          description: 'Cómo administrar y reconocer contribuciones al Bien Común de manera efectiva',
          teacherId: 'teacher2',
        ...
        },
        {
          ...
          description: 'Fundamentos del Local Exchange Trading System y su aplicación en CoomÜnity',
          teacherId: 'teacher3',
          ...
          scheduledAt: new Date(Date.now() + 10 * 24 * 60 * 60 * 1000).toISOString(), // En 10 días
          status: 'scheduled'
        }
      ];
...
//     console.log('>>> LetsService.createKnowledgeExchange: Creating knowledge exchange', exchangeData);
    ...
    try {
      ...
      if (exchangeData.unitsCost < 0) {
        throw new BadRequestException('El costo en Ünits no puede ser negativo');
      }
      ...
      const newExchange = {
        id: `ke_${Date.now()}`,
        ...exchangeData,
        teacherName: teacher.firstName ? `${teacher.firstName} ${teacher.lastName || ''}`.trim() : teacher.email,
      ...
      const mockNotifications = [
        {
          ...
          message: 'Has recibido 25 Ünits de María González por la sesión de Ayni',
        ...
        },
        {
          ...
          message: 'Tienes nuevos intercambios recomendados basados en tu actividad',
