import { Controller, Get, Post, Body, Patch, Param, UseGuards, Req, ForbiddenException, NotFoundException } from '@nestjs/common';
...
export class TransactionsController {
  ...
  async findAllForUser(@Req() req: AuthenticatedRequest, @Param('userId') userId: string) {
      // Check if the authenticated user is requesting their own data OR is an admin
      if (req.user.id !== userId && !req.user.roles.includes('admin')) {
          throw new ForbiddenException('You do not have permission to view transactions for this user.');
      }
  ...
  @Get(':id')
  @ApiOperation({ summary: 'Get a specific transaction by ID (Owner or Admin only)' })
  ...
  async findOne(@Req() req: AuthenticatedRequest, @Param('id') id: string) {
      ...
      const transaction = await this.transactionsService.findTransaction(id, req.user);
      if (!transaction) {
  ...
  @ApiResponse({ status: 200, description: 'Transaction details (Admin only).' })
  @ApiResponse({ status: 404, description: 'Transaction not found.' })
  @ApiResponse({ status: 403, description: 'Forbidden resource.' })
  @ApiParam({ name: 'id', description: 'ID of the transaction' })
