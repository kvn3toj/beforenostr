...
async function seedMarketplace(prisma) {
  ...
  const marketplaceItems = [
    ...
    },
    {
      ...
      currency: 'UNITS',
      status: 'ACTIVE',
      images: ['https://images.unsplash.com/photo-1620656272587-3f3a88647c43?w=600'],
      tags: ['limpieza energética', 'ritual', 'palo santo', 'salvia', 'ético'],
      location: 'Envíos a todo el país',
      sellerId: creatorUser?.id || users[0]?.id,
      metadata: JSON.stringify({
        contents: ['1 atado de salvia blanca', '3 palos de palo santo', 'Guía de uso'],
        origin: 'Cultivo sostenible en Perú y California'
      }),
    },
    {
        name: 'Asesoría Personalizada en Astrología Evolutiva',
        description: 'Una sesión de 90 minutos para explorar tu carta natal y entender tus patrones evolutivos.',
        itemType: 'SERVICE',
        price: 90,
        priceToins: 40,
        currency: 'UNITS',
        status: 'ACTIVE',
        images: ['https://images.unsplash.com/photo-1590212154839-e5399a9a4563?w=600'],
        ...
        metadata: JSON.stringify({
          ...
          includes: ['Grabación de la sesión', 'PDF con claves de la carta natal']
        }),
      },
  ];
  ...
  for (const item of marketplaceItems) {
    ...
    if (!existingItem) {
      ...
      console.log(`   - Marketplace item already exists, skipping: ${item.name}`);
    }
  }
