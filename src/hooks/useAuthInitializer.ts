...
export const useAuthInitializer = () => {
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      ...
      console.log('[AuthInitializer] Suscripción a onAuthStateChange creada.');

      // Marcar como inicializado solo después de todo
      setIsInitialized(true);
      console.log('[AuthInitializer] Inicialización marcada como completada.');

      // Devolver función de limpieza
      return () => {
        console.log('[AuthInitializer] Limpiando listener de onAuthStateChange...');
        subscription.unsubscribe();
      };
    };

    // Llamar a initialize y manejar su limpieza
    let cleanup: (() => void) | undefined;
    initialize().then(cleanupFn => {
        cleanup = cleanupFn;
    });

    // Limpieza del useEffect principal (se ejecuta si el componente se desmonta ANTES de que initialize termine)
    return () => {
        console.log('[AuthInitializer] Limpieza principal del useEffect.');
        if (cleanup) {
            cleanup(); // Llama a la limpieza de la suscripción si ya se creó
        }
    };

  }, [setSession, setUser]); // Dependencias del useEffect
